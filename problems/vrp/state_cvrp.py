...
class StateCVRP(NamedTuple):
    ...
    def partial_finished(self):
        ...
        return (not torch.any(self.prev_a[:, -1])) and self.i != 0
    ...
    def get_mask(self, attention_neighborhood=0, pause_on_depots=False):
        ...
        if attention_neighborhood != 0:
            ...
            neighbors_cutoff = torch.topk(current_dis_vector, attention_neighborhood, largest=False)[0][:, :,
                               -1:].reshape(mask_loc.shape[0], -1)
        ...
        if pause_on_depots:
            if (not self.partial_finished()) and self.i != 0:
