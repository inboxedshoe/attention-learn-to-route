...
class DynamicAttentionEncoder(BaseEncoder):
    ...
    def create_layers(self, **kwargs):
        ...
        # embed nur coords depot
        self.init_embed_depot = nn.Linear(2, self.hidden_dim)
        # embed coords + demand
        self.init_embed = nn.Linear(3, self.hidden_dim)
    ...
    def forward(self, input, attn_mask=None, current_num_nodes=None, key_padding_mask=None):
        ...
        if attn_mask is not None:
            output = (h, torch.sum(h, dim=1)/current_num_nodes)
        else:
            output = (h, torch.mean(h, dim=1))
